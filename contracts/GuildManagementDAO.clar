...
(define-constant err-insufficient-stake (err u200))
...
;; Helper: return current stake for a principal or zero (returns uint)
(define-read-only (internal-get-stake (p principal))
  (let ((opt (map-get? member-stakes p)))
    (if (is-none opt) u0 (unwrap! opt u0))))
...
(define-public (stake-tokens (amount uint))
  (let ((caller tx-sender))
    (begin
      ...
      ;; transfer guild tokens from caller to contract to represent stake
      (try! (ft-transfer? guild-token amount caller (as-contract tx-sender)))
      ;; update member stake and total
      (let ((old (internal-get-stake caller)))
        (map-set member-stakes caller (+ old amount)))
...
(define-public (unstake-tokens (amount uint))
  (let ((caller tx-sender))
    (begin
      (asserts! (> amount u0) err-invalid-amount)
      (let ((old (internal-get-stake caller)))
        (asserts! (>= old amount) err-insufficient-stake)
        ;; decrease stake
        (map-set member-stakes caller (- old amount))
        (var-set total-staked (- (var-get total-staked) amount))
        ;; transfer guild tokens back to member
        (try! (ft-transfer? guild-token amount (as-contract tx-sender) caller))
        (ok true)))))

(define-read-only (get-staked-balance (member principal))
  (internal-get-stake member))